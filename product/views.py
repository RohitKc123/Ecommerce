...
def product_list(request):
    ...
    data1 = Product.objects.all().order_by("-added_date").exclude(quantity=0)
    num = 4
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if quantity <= product.quantity:
                ...
                product.quantity = product.quantity - int(quantity) + int(old_quantity)
                product.save()
                item.save()
            else:
...
def latest_product_page(request):
    latest_product = Product.objects.all().exclude(quantity=0).order_by("-added_date")
